...
def es_write_data(es_client: AsyncElasticsearch):  # INFO запись в index movies
...
# def make_get_request(aiohttp_client: aiohttp_client.ClientSession):
def make_get_request():
    async def inner(url:str, query_data:dict):

        async with aiohttp.ClientSession() as session:
            response = await session.get(url=url, params=query_data)
